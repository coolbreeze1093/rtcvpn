            ...
            [this, self, resolver](std::error_code ec,
                ...
                asio::async_connect(
                    ...
                    [this, self](std::error_code ec, const tcp::endpoint&) {
                        if (ec) {
                            std::cerr << "[远端] 连接目标失败 " << ec.message()
                                      << "\n";
                            mux_.send_synack(session_->stream_id(), false);
                            close();return;
                            return;
                        }
                        std::cout << "[远端] 已连接目标, stream_id="
                                  << session_->stream_id() << "\n";
...
private:
    void setup_session_callbacks() {
        ...
        session_->set_on_data([this, self](const uint8_t* d, size_t n) {
            ...
            to_target_queue_.emplace_back(d, d + n);
            if (!writing) do_write_to_target();
        });
    ...
    void do_write_to_target() {
        ...
        asio::async_write(
            ...
            [this, self](std::error_code ec, std::size_t) {
                ...
                if (!to_target_queue_.empty()) do_write_to_target();
            });
    }
